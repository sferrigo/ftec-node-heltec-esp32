...
void do_send(osjob_t* j) {
  ...
  int vibracao = (rand()%100);
  //vibracao = vibracao/100;
  int ruido = 78 + (rand()%27);
  if (vibracao > 70 && !lock){
    predicao--;
    lock = true;
  }
  ...
  if (LMIC.opmode & OP_TXRXPEND) {
    ...
    //Se não medir temperatura e umidade escreve msg de erro
      ...
      myString = myString + String(vibracao);
    ...
    lpp.addDigitalInput(4, vibracao);
    lpp.addDigitalInput(5, predicao);
    lpp.addDigitalInput(6, ruido);
    lpp.addGPS(7, -29.402010, -50.865897, 620);
    lpp.addDigitalInput(8, 83);
    #else
